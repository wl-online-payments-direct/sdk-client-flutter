...
void main() {
        ...
        switch (methodCall.method) {
          ...
          case "getPaymentProduct":
            final PaymentProduct paymentProduct = PaymentProduct.fromJson(
                await jsonMap(
                    "test/mock_responses/payment_product_response.json"));
            return jsonString(SdkResult(paymentProduct, null));
          case "getPaymentProductNetworks":
            final PaymentProductNetworks paymentProductNetworks =
          ...
          case "getSurchargeCalculation":
            final SurchargeCalculation surchargeCalculation =
          ...
          case "preparePaymentRequest":
            final PreparedPaymentRequest preparedPaymentRequest =
                PreparedPaymentRequest.fromJson(await jsonMap(
                    "test/mock_responses/prepared_payment_request.json"));
  ...
  test('getIinDetails', () async {
    ...
    await session.getIinDetails(
        ...
        listener: IinLookupResponseListener(onSuccess: (response) {
          ...
          fail(
              "apiError occurred when retrieving IinDetails: ${apiError?.message}");
        }, onException: (throwable) {
          fail(
              "throwable occurred when retrieving IinDetails: ${throwable?.error}");
        }));
  });
  ...
  test('getBasicPaymentProducts', () async {
    ...
    await session.getBasicPaymentProducts(
        ...
        listener: BasicPaymentProductsResponseListener(onSuccess: (response) {
          ...
          fail(
              "apiError occurred when retrieving BasicPaymentProducts: ${apiError?.message}");
        }, onException: (throwable) {
          fail(
  ...
  test('getPaymentProduct', () async {
    ...
    await session.getPaymentProduct(
        ...
        listener: PaymentProductResponseListener(onSuccess: (response) {
          ...
          fail(
              "apiError occurred when retrieving PaymentProduct: ${apiError?.message}");
        }, onException: (throwable) {
          fail(
  ...
  test('getPaymentProductNetworks', () async {
    ...
    await session.getPaymentProductNetworks(
        ...
        listener: PaymentProductNetworksResponseListener(onSuccess: (response) {
          ...
          fail(
              "apiError occurred when retrieving PaymentProductNetworks: ${apiError?.message}");
        }, onException: (throwable) {
          fail(
              "throwable occurred when retrieving PaymentProductNetworks: ${throwable?.error}");
        }));
  });

  test('getSurchargeCalculation', () async {
    final request = SurchargeCalculationRequest.withToken(
        AmountOfMoney(1000, "USD"), "SurchargeCalculationRequestToken");
    ...
    await session.getSurchargeCalculation(
        ...
        listener: SurchargeCalculationResponseListener(onSuccess: (response) {
          ...
          fail(
              "apiError occurred when retrieving SurchargeCalculation: ${apiError?.message}");
        }, onException: (throwable) {
          fail(
              "throwable occurred when retrieving SurchargeCalculation: ${throwable?.error}");
        }));
  });
  ...
  PaymentRequest createPaymentRequest() {
    ...
    final PaymentProductDisplayHints dummyDisplayHints =
        PaymentProductDisplayHints("VISA",
            displayOrder: 0, logoUrl: "VISA_logo");
    final PaymentProduct dummyPaymentProduct = PaymentProduct(
        "1", "card", "cards", false, false, false,
  ...
  test('preparePaymentRequest', () async {
    ...
    await session.preparePaymentRequest(
        ...
        listener: PaymentRequestPreparedListener(onSuccess: (response) {
          ...
          fail(
              "apiError occurred when preparing PaymentRequest: ${apiError?.message}");
        }, onException: (throwable) {
          fail(
              "throwable occurred when preparing PaymentRequest: ${throwable?.error}");
        }));
  });
}
...
Future<Session> createSession() async {
  return Future.value(Session(
      "_clientSessionId", "_customerId", "_clientApiUrl", "_assetUrl",
      loggingEnabled: true));
}
