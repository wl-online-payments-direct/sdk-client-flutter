...
final class Session {
  ...
  SdkSessionRequest get _initSessionRequest => SdkSessionRequest(_clientSessionId, _customerId, _clientApiUrl, _assetUrl, isEnvironmentProduction, _getAppIdentifier(), loggingEnabled);
  ...
  Session(this._clientSessionId, this._customerId, this._clientApiUrl, this._assetUrl, {
      this.isEnvironmentProduction = false,
      this.appIdentifier = "${Constants.appIdentifierPrefix}/UnknownAppId",
      this.loggingEnabled = false
  })
      : assert(
          _clientSessionId.isNotEmpty, "Missing clientSessionId"
        ),
        assert(
          _customerId.isNotEmpty, "Missing customerId"
        ),
        assert(
          _clientApiUrl.isNotEmpty, "Missing clientApiUrl"
        ),
        assert(
          _assetUrl.isNotEmpty, "Missing assetUrl"
        )
  {
    _initNativeSdk();
  }
  ...
  Future<void> _initNativeSdk() async {
    ...
    final result = await
      _nativeSdkCommunicator.createSession(sessionRequest).onError((error, stackTrace) {
  ...
  getPublicKey({
    required PublicKeyResponseListener listener
  }) {
  ...
  getIinDetails({
    ...
    return _nativeSdkCommunicator.listenForIinDetails(request, listener).awaitJob();
  }
  ...
  getBasicPaymentProducts({
    ...
    return _nativeSdkCommunicator.listenForBasicPaymentProducts(request, listener).awaitJob();
  }
  ...
  getPaymentProduct({
    ...
    return _nativeSdkCommunicator.listenForPaymentProduct(request, listener).awaitJob();
  }

  /// Retrieves a payment product networks as a [PaymentProductNetworks] object from the Online Payments gateway with the use of a [PaymentProductNetworksRequest].
  getPaymentProductNetworks({
    ...
    return _nativeSdkCommunicator.listenForPaymentProductNetworks(request, listener).awaitJob();
  }
  ...
  getSurchargeCalculation({
    ...
    return _nativeSdkCommunicator.listenForSurchargeCalculation(request, listener).awaitJob();
  }
  ...
  preparePaymentRequest({
    ...
    return _nativeSdkCommunicator.listenForPreparedPaymentRequest(request, listener).awaitJob();
  }
}