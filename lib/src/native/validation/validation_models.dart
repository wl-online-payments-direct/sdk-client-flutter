...
final class PaymentRequestValidator {
  static Future<List<ValidationErrorMessage>> validatePaymentRequest(
    ...
    final validationResultStr =
        await _NativeValidationInterface.instance.validatePaymentRequest(jsonEncode(validationRequest));

    return _parseValidationErrorMessages(validationResultStr);
  }

  static Future<List<ValidationErrorMessage>> validatePaymentProductFieldForPaymentRequest(
    String fieldId,
    ...
    final validationResultStr = await _NativeValidationInterface.instance
        .validatePaymentProductFieldForPaymentRequest(jsonEncode(validationRequest));
...
final class PaymentProductFieldValidator {
  static Future<List<ValidationErrorMessage>> validateValueForPaymentProductField(
    String value,
    PaymentProductField field,
    PaymentProduct paymentProduct,
  ) async {
    final validationRequest = PaymentProductFieldValidationRequest(value, field, paymentProduct);
    final validationResultStr =
        await _NativeValidationInterface.instance.validateValueForPaymentProductField(jsonEncode(validationRequest));
...
final class ValidationRuleValidator {
  static Future<List<ValidationErrorMessage>> validateValueForValidationRule(
    ...
    final validationResultStr =
        await _NativeValidationInterface.instance.validateValueForValidationRule(jsonEncode(validationRequest));

    return _parseValidationErrorMessages(validationResultStr);
  }

  static Future<List<ValidationErrorMessage>> validateValidationRuleForPaymentRequestAndFieldId(
    PaymentRequest request,
    String fieldId,
    ValidationRule rule,
  ) async {
...
List<ValidationErrorMessage> _parseValidationErrorMessages(
  ...
  final validationResult = json.map((vem) => ValidationErrorMessage.fromJson(vem));

  return validationResult.toList(growable: false);
}
